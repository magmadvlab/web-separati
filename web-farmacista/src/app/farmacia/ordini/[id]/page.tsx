...
export default function FarmaciaDettaglioOrdinePage() {
  ...
  // ── Decrittazione prescrizione (RSA hybrid) ──────────────────────
  const decryptMutation = useMutation({
    mutationFn: async () => {
      const response = await api.get(
        `/farmacia/ordini/${ordineId}/prescrizione/decriptata`
      );
      return response.data?.data || response.data;
    },
    onSuccess: (data: any) => {
      toast({
        title: "Prescrizione decifrata",
        description: "I dati medici sono stati decifrati con successo",
      });
    },
    onError: (error: any) => {
      toast({
        ...
        description:
          ...
          "Impossibile decifrare la prescrizione. Verifica che le chiavi RSA siano configurate.",
        variant: "destructive",
      });
    },
  });

  const prescrizioneDecifrata = decryptMutation.data;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-2 space-y-6">
            ...
            <Card className="border-green-200 bg-green-50/30">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-normal text-green-600 bg-green-100 px-2 py-1 rounded ml-2">OTC</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {(ordine as any).farmaciDaBanco.map((farmaco: any, index: number) => (
                    <div key={index} className="p-4 rounded-lg border border-green-200 bg-white">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="text-right ml-4">
                          <p className="font-medium text-lg">Quantita: {farmaco.quantita || 1}</p>
                          {farmaco.prezzo && (
                            <p className="text-sm text-gray-600 mt-1">
                              {"\u20AC"}{Number(farmaco.prezzo * (farmaco.quantita || 1)).toFixed(2)}
                            </p>
          ...
          {/* Foto Riferimento */}
          {(ordine as any).fotoRiferimento && (
            <Card className="border-blue-200 bg-blue-50/30">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <ImageIcon className="h-5 w-5 text-blue-600" />
                  Foto Riferimento Paziente
                </CardTitle>
              </CardHeader>
              <CardContent>
                <a
                  href={(ordine as any).fotoRiferimento}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block"
                >
                  <img
                    src={(ordine as any).fotoRiferimento}
                    alt="Foto riferimento farmaco"
          ...
          {/* Dati Medici Cifrati (RSA) — Solo farmacia può decifrare */}
          {ordine.prescrizioneId && (
            <Card className="border-purple-200 bg-purple-50/30">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  {prescrizioneDecifrata ? (
                    <ShieldCheck className="h-5 w-5 text-green-600" />
                  ) : (
                    <Lock className="h-5 w-5 text-purple-600" />
                  )}
                  Dati Medici Prescrizione
                  {prescrizioneDecifrata?.decriptata && (
              ...
              <CardContent>
                {!prescrizioneDecifrata ? (
                  <div className="text-center py-6">
                    <Lock className="h-10 w-10 text-purple-300 mx-auto mb-3" />
                    <p className="text-sm text-gray-600 mb-1">
                      I dati medici di questa prescrizione sono cifrati end-to-end.
                    </p>
                    <p className="text-xs text-gray-500 mb-4">
                      Solo la tua farmacia può decifrarli con la chiave privata RSA-4096.
                    </p>
                    <Button
                    ...
                    >
                      ...
                      {decryptMutation.isPending ? "Decifratura in corso..." : "Decifra Prescrizione"}
                    </Button>
                    {decryptMutation.isError && (
                      <p className="text-sm text-red-600 mt-3">
                        <AlertCircle className="h-3 w-3 inline mr-1" />
                        Errore nella decrittazione. Contatta il supporto tecnico.
                      </p>
                    )}
                  </div>
                ) : (
                  <div className="space-y-4">
                    {/* Codice NRE */}
                    {prescrizioneDecifrata.codiceNre && (
                      <div className="flex items-center gap-2 p-3 bg-white rounded-lg border">
                        <FileText className="h-4 w-4 text-purple-500" />
                        <div>
                          <p className="text-xs text-gray-500">Codice NRE</p>
                          <p className="font-mono font-medium">{prescrizioneDecifrata.codiceNre}</p>
                        </div>
                      </div>
                    )}

                    {/* Farmaci decifrati */}
                    {prescrizioneDecifrata.farmaci && Array.isArray(prescrizioneDecifrata.farmaci) && (
                      <div>
                        <p className="text-sm font-medium text-gray-700 mb-2">
                          Farmaci Prescritti ({prescrizioneDecifrata.farmaci.length})
                        </p>
                        <div className="space-y-2">
                          {prescrizioneDecifrata.farmaci.map((farmaco: any, idx: number) => (
                            <div key={idx} className="p-3 bg-white rounded-lg border border-purple-100">
                              <p className="font-medium">{farmaco.nomeFarmaco || farmaco.nome || "Farmaco"}</p>
                              {farmaco.principioAttivo && (
                                <p className="text-sm text-gray-600">P.A.: {farmaco.principioAttivo}</p>
                              )}
                              {farmaco.posologia && (
                                <p className="text-sm text-gray-600">Posologia: {farmaco.posologia}</p>
                              )}
                              {farmaco.quantita && (
                                <p className="text-sm text-gray-600">Quantità: {farmaco.quantita}</p>
                              )}
                            </div>
                          ))}
                        </div>
                    ...
                    {/* Note medico */}
                    {prescrizioneDecifrata.noteMedico && (
                      <div className="p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                        <p className="text-xs text-yellow-700 font-medium mb-1">Note del Medico</p>
                        <p className="text-sm">{prescrizioneDecifrata.noteMedico}</p>
                      </div>
                    )}

                    {/* Badge metodo crittografia */}
                    {prescrizioneDecifrata.metodoDecrittazione && (
                      <div className="flex items-center gap-1 text-xs text-gray-400 pt-2 border-t">
                        <ShieldCheck className="h-3 w-3" />
                        Decifrata con {prescrizioneDecifrata.metodoDecrittazione}
                      </div>
                    )}
                  </div>
                )}
