...
export default function CalendarioFarmaciaPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [dispRes, eccRes] = await Promise.all([
        fetch('/api/farmacia/calendario/disponibilita', {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch('/api/farmacia/calendario/eccezioni', {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const handleAddDisponibilita = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('/api/farmacia/calendario/disponibilita', {
        method: 'POST',
        headers: {
  ...
  const handleAddEccezione = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('/api/farmacia/calendario/eccezioni', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteDisponibilita = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/farmacia/calendario/disponibilita/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteEccezione = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/farmacia/calendario/eccezioni/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Gestione Orari Farmacia</h1>
          <p className="mt-2 text-gray-600">
            Configura i tuoi orari di apertura e gestisci ferie/chiusure straordinarie
          </p>
        </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            {disponibilita.length === 0 ? (
              <p className="text-gray-500 text-center py-8">
                Nessun orario configurato. Aggiungi i tuoi orari di apertura.
              </p>
            ) : (
              <div className="space-y-3">
                {disponibilita
                  ...
                  .map((disp) => (
                    <div
                      key={disp.id}
                      className="border border-gray-200 rounded-lg p-4 hover:border-green-300"
                    >
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            Slot: {disp.durataSlot} min • Max {disp.postiPerSlot} servizio/i
                          </div>
                          {disp.note && (
        ...
        <div className="mt-8 text-center">
          <button
            onClick={() => router.push('/farmacia/ordini')}
