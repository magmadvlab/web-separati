...
export default function AdminProfessionistiPage() {
  ...
  const toggleDisponibilita = async (id: number, disponibile: boolean) => {
    try {
      const response = await fetch(`/api/professionisti/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ disponibile: !disponibile })
      });
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-600">
                  {(professionisti.reduce((acc, p) => acc + (p.valutazioneMedia || 0), 0) / professionisti.length).toFixed(1)}
                </p>
              </div>
                  ...
                  <tr key={prof.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {prof.valutazioneMedia ? (
                        <div>
                          <div className="text-sm font-medium text-gray-900">
                            {prof.valutazioneMedia.toFixed(1)} ⭐
                          </div>
                          <div className="text-sm text-gray-500">
