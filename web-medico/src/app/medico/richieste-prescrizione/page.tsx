...
export default function MedicoRichiestePrescrizionePage() {
  ...
  const richiesteFiltrate = richieste?.filter((richiesta) => {
    if (ricerca) {
      ...
      const matchFarmaci = richiesta.farmaciRichiesti.some((f) =>
        f.nomeFarmaco.toLowerCase().includes(searchLower)
      );
      return matchPaziente || matchFarmaci;
    }
    return true;
  }) || [];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Richieste Prescrizione</h1>
          <p className="text-gray-600 mt-1">
            Gestisci le richieste di prescrizione dai tuoi pazienti
          </p>
        </div>
        {richiesteInAttesa > 0 && (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Farmaci Richiesti</TableHead>
                  ...
                  <TableRow key={richiesta.id}>
                    ...
                    <TableCell>
                      <div className="max-w-xs">
                        {Array.isArray(richiesta.farmaciRichiesti) &&
                        richiesta.farmaciRichiesti.length > 0 ? (
                          <div className="space-y-1">
                            {richiesta.farmaciRichiesti.slice(0, 2).map((farmaco, idx) => (
                              <p key={idx} className="text-sm">
                                {farmaco.nomeFarmaco}
                                {farmaco.numeroScatole && ` (${farmaco.numeroScatole} scatole)`}
                              </p>
                            ))}
                            {richiesta.farmaciRichiesti.length > 2 && (
                              <p className="text-xs text-gray-500">
                                +{richiesta.farmaciRichiesti.length - 2} altri
                              </p>
                            )}
                          </div>
                        ) : (
                          <span className="text-sm text-gray-400">Nessun farmaco</span>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
