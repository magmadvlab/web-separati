...
export default function MedicoDettaglioRichiestaPrescrizionePage() {
  ...
  const [posologieForm, setPosologieForm] = useState<Record<string, string>>({});
  ...
  const isAttesaPosologia = richiesta?.stato === "attesa_posologia";

  const handleApprova = () => {
    // Per richieste da email (attesa_posologia), la posologia viene dal form
    const farmaciFormattati = farmaciRichiesti.map((farmaco) => {
      const key = farmaco.principioAttivo || farmaco.nomeFarmaco || "";
      return {
        farmacoId: farmaco.farmacoId || null,
        nomeFarmaco: farmaco.nomeFarmaco || farmaco.principioAttivo || "",
        numeroScatole: farmaco.numeroScatole || 1,
        compressePerScatola: farmaco.compressePerScatola || 30,
        posologia: posologieForm[key] || farmaco.posologia || "",
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-3 space-y-1">
                        ...
                        {farmaco.doseGiornaliera && (
                          <p className="text-sm">
                            <span className="font-medium">Dose giornaliera totale:</span>{" "}
                            {farmaco.doseGiornaliera}
                          </p>
                        )}
                        {Array.isArray(farmaco.orariAssunzione) && farmaco.orariAssunzione.length > 0 && (
                          <p className="text-sm">
                            <span className="font-medium">Orari assunzione:</span>{" "}
                            {farmaco.orariAssunzione.join(", ")}
                          </p>
                        )}
                        {farmaco.conPasto === true && (
                          <p className="text-sm">
                            <span className="font-medium">Assunzione:</span> con pasto
                          </p>
                        )}
                        {farmaco.motivo && (
                          <p className="text-sm">
