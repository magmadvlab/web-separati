...
export default function CalendarioMedicoPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [dispRes, eccRes] = await Promise.all([
        fetch('/api/medico/calendario/disponibilita', {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch('/api/medico/calendario/eccezioni', {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const handleAddDisponibilita = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('/api/medico/calendario/disponibilita', {
        method: 'POST',
        headers: {
  ...
  const handleAddEccezione = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('/api/medico/calendario/eccezioni', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteDisponibilita = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/medico/calendario/disponibilita/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteEccezione = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/medico/calendario/eccezioni/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            Configura i tuoi orari di visita e gestisci ferie/chiusure
          </p>
        </div>
