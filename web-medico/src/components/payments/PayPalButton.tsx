...
function PayPalButtonsInner({ ordineId, totale, onSuccess, onError }: PayPalButtonProps) {
  ...
  const createOrder = async () => {
    try {
      // Crea payment intent sul backend (per tracciamento)
      const response = await api.post("/payments/create-intent", {
        ordineId,
        metodoPagamento: "paypal",
      });

      // Per PayPal, creiamo l'ordine direttamente con PayPal SDK
      // Il backend dovrebbe creare un ordine PayPal e restituire l'ID
      // Per ora, usiamo un approccio semplificato
      return `order_${ordineId}_${Date.now()}`;
    } catch (error: any) {
      const errorMessage = error.response?.data?.message || error.message || "Errore nella creazione dell'ordine";
      toast({
        title: "Errore",
