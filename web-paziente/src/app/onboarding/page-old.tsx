...
export default function OnboardingPage() {
  ...
  // Commento temporaneo per testing - riabilitare in produzione
            ...
            <div className="space-y-6 text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
                ...
                <div className="p-4 bg-green-50 rounded-lg">
                  ...
                  <h4 className="font-semibold text-gray-900 mb-1">Aggiungi i tuoi animali</h4>
                  <p className="text-sm text-gray-600">Servizi anche per i tuoi amici a 4 zampe</p>
                </div>
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div className="text-center mb-6">
                ...
                <p className="text-gray-600">
                  RicettaZero offre anche servizi veterinari per i tuoi amici a 4 zampe
                </p>
              </div>
              ...
              {hasAnimals === true && (
                <>
                  <AnimaleForm
                    animali={animali}
                    onAdd={(animale) => setAnimali([...animali, animale])}
                    onRemove={(index) => setAnimali(animali.filter((_, i) => i !== index))}
                  />

                  <div className="flex gap-4">
                    <Button variant="outline" onClick={() => setHasAnimals(null)}>
                      <ArrowLeft className="mr-2 h-4 w-4" />
                      Indietro
                    </Button>
                    <Button onClick={handleNext} className="flex-1" disabled={animali.length === 0}>
                      Continua
                      <ArrowRight className="ml-2 h-4 w-4" />
                    </Button>
                  </div>
          ...
          {step === 6 && (
            <div className="space-y-6">
              <div className="text-center mb-6">
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-2">
                  Carica i talloncini delle tue terapie
                </h3>
                <p className="text-gray-600">
                  Opzionale: carica le foto dei talloncini per importare le tue terapie attuali
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                {uploadedPhotos.length > 0 && (
                  <div className="space-y-2">
                    <Label>Foto caricate ({uploadedPhotos.length})</Label>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {uploadedPhotos.map((photo, index) => (
                        <div key={index} className="relative">
                          <img
                            src={photo}
                            alt={`Talloncino ${index + 1}`}
                ...
                <div className="space-y-3">
                  ...
                  {animali.length > 0 && (
                    <div className="flex items-center gap-3">
                      <CheckCircle2 className="h-5 w-5 text-green-600" />
                      <div>
                        <p className="font-medium text-gray-900">Animali registrati</p>
                        <p className="text-sm text-gray-600">
                          {animali.map(a => a.nome).join(', ')}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {uploadedPhotos.length > 0 && (
                    <div className="flex items-center gap-3">
                      <CheckCircle2 className="h-5 w-5 text-green-600" />
                      <div>
                        <p className="font-medium text-gray-900">Talloncini caricati</p>
                        <p className="text-sm text-gray-600">{uploadedPhotos.length} foto</p>
                      </div>
                    </div>
                  )}
                </div>
