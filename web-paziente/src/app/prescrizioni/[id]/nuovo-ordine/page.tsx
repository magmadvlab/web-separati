...
export default function NuovoOrdinePage() {
  ...
  // Guard: le prescrizioni SSN vengono gestite automaticamente dalla pipeline.
  // Il paziente non deve (e non può) creare l'ordine manualmente per ricette SSN.
  // Solo gli ordini OTC (senza NRE) passano per questo form.
  useEffect(() => {
    if (prescrizione && prescrizione.codiceNre) {
      toast({
        title: "Ordine già in elaborazione",
        description:
          "La tua prescrizione SSN viene gestita automaticamente. " +
          "Controlla i tuoi ordini per lo stato di avanzamento.",
      });
      router.replace(`/prescrizioni/${prescrizioneId}`);
    }
  }, [prescrizione]);
  ...
  const handleSubmit = () => {
    ...
    createOrdineMutation.mutate({
      ...
      farmaciaId: farmaciaSelezionata || undefined,
      modalitaConsegna: "batch",
      tipoConsegna,
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-sm text-gray-600 mb-3">
                Delivery raggruppera gli ordini e scegliera la farmacia del batch. Questa lista e solo informativa.
              </p>
