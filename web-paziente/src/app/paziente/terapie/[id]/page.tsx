...
const getPrescriptionBadgeState = (
  terapia?: Terapia | null,
): "prescription" | "otc" | "unknown" => {
  if (!terapia) return "unknown";

  const hasPrescriptionLink = Boolean(terapia.prescrizioneId || terapia.prescrizione?.id);
  const ricettaRichiesta = terapia.farmaco?.ricettaRichiesta;

  if (ricettaRichiesta === true || hasPrescriptionLink) {
    return "prescription";
  }

  if (ricettaRichiesta === false) {
    return "otc";
  }
...
export default function DettaglioTerapiaPage() {
  ...
  const prescriptionBadgeState = getPrescriptionBadgeState(terapia);
        ...
        <Card>
          <CardHeader>
            <div className="flex items-start justify-between">
              ...
              {prescriptionBadgeState === "prescription" ? (
                <span className="text-xs font-medium text-red-600 bg-red-100 px-2 py-1 rounded">
                  <FileText className="h-3 w-3 inline mr-1" />
                  Richiede Ricetta
                </span>
              ) : prescriptionBadgeState === "otc" ? (
                <span className="text-xs font-medium text-green-600 bg-green-100 px-2 py-1 rounded">
                  OTC
                </span>
              ) : (
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {terapia.richiestaRinnovoInCorso ? (
              ...
              <div className={`p-3 rounded border ${terapia.statoReminder === 'critical' ? 'bg-red-50 border-red-200' : 'bg-yellow-50 border-yellow-200'}`}>
                <p className={`text-sm font-medium ${terapia.statoReminder === 'critical' ? 'text-red-800' : 'text-yellow-800'}`}>
                  {terapia.messaggioReminder}
                </p>
                {terapia.azioneRichiesta === 'rinnovo' && (
                  ...
                  >
                    ...
                    {richiestaRinnovoMutation.isPending ? "Invio in corso..." : "Richiedi Rinnovo al Medico"}
                  </Button>
                )}
                {terapia.azioneRichiesta === 'ritiro' && (
                  <p className="text-xs mt-1 text-yellow-700">Puoi ritirare direttamente in farmacia con la ricetta.</p>
                )}
              </div>
            ) : null}
          </CardContent>
