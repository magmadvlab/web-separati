      ...
      if (selectedFarmaco?.ricettaRichiesta) {
        ...
        const richiestaData = {
          ...
          farmaciRichiesti: [{
            ...
            compressePerScatola: data.compressePerScatola,
            posologia: data.posologia,
            doseGiornaliera: data.doseGiornaliera,
            orariAssunzione: orari.filter((o) => o.trim().length > 0),
            conPasto: data.conPasto,
            formaFarmaceutica: selectedFarmaco.formaFarmaceutica,
            unitaMisura: selectedFarmaco.unitaMisura,
            note: data.note || "",
          }],
          motivo: `Richiesta prescrizione per ${selectedFarmaco.nomeCommerciale}`,
          notePaziente: `Posologia proposta: ${data.posologia}. Dose giornaliera totale: ${data.doseGiornaliera}. Orari assunzione: ${orari.filter((o) => o.trim().length > 0).join(", ")}. ${data.conPasto ? "Assunzione con pasto." : ""}`,
        };
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="Es. 1/2 al mattino e 1/2 la sera, oppure 1/4 due volte al giorno"
                            {...field}
                          />
                        </FormControl>
                        <FormDescription>
                          Descrivi esattamente la ripartizione della dose nelle varie assunzioni
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Dose Giornaliera Totale *</FormLabel>
                        <FormControl>
                          <Input
                            type="number"
                            step="0.01"
                            min="0.1"
                            placeholder="Es. 1, 0.5, 0.25"
                            {...field}
                            onChange={(e) => field.onChange(parseFloat(e.target.value) || 0)}
                          />
                        </FormControl>
                        <FormDescription>
                          Quantità totale al giorno, anche frazionaria. Usa la posologia per spiegare come dividerla nelle singole assunzioni.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
