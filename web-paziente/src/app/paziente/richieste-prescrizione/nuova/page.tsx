...
import { Camera, Upload, Plus, X, FileText, Pill, AlertCircle, Stethoscope } from "lucide-react";
import { motion } from "framer-motion";
import { Alert, AlertDescription } from "@/components/ui/alert";
import type { ApiResponse } from "@/types/api";

const formSchema = z.object({
  tipoRichiesta: z.enum(["farmaci", "farmaci_da_banco", "accertamenti_esami"], {
...
export default function NuovaRichiestaPrescrizione() {
  ...
  const isAccertamenti = tipoRichiesta === "accertamenti_esami";
  const isFarmaciDaBanco = tipoRichiesta === "farmaci_da_banco";
  ...
  const handleFileUpload = async (file: File) => {
    ...
    if (isAccertamenti) {
      if (!isValidImage && !isValidDocument) {
        toast({
          title: "Errore",
          description: "Carica un file valido (PDF, DOC, DOCX, immagine, ZIP)",
          variant: "destructive",
        });
        return;
      }
    ...
    const maxSizeMb = isAccertamenti ? 50 : 10;
    if (file.size > maxSizeMb * 1024 * 1024) {
      toast({
        title: "Errore",
        description: `Il file è troppo grande (max ${maxSizeMb}MB)`,
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const uploadEndpoint = isAccertamenti
        ? "/paziente/upload/cartella-clinica/documento"
      ...
      setUploadedFileName(response.data.data?.originalName || file.name);
      setUploadedFileMimeType(response.data.data?.mimeType || file.type);
      form.setValue("fotoTalloncino", photoUrl);

      if (!isAccertamenti && response.data.data?.farmaci && response.data.data.farmaci.length > 0) {
      ...
      } else if (isAccertamenti) {
        toast({
          title: "Documento caricato",
          description: "La documentazione è stata allegata alla richiesta per il medico",
        });
      } else {
        toast({
          title: "Foto caricata",
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      const profiloResponse = await api.get<ApiResponse<any>>("/paziente/profile");
      const medicoId = profiloResponse.data?.data?.medicoCuranteId;

      if (!medicoId) {
        throw new Error("Nessun medico curante associato al tuo profilo");
      }

      const righeRichiesta = data.farmaci.map((item) => ({
        nomeFarmaco: item.nome,
        numeroScatole: isAccertamenti ? 1 : item.quantita,
        posologia: item.dosaggio || (isAccertamenti ? "Accertamento / esame specifico" : ""),
        doseGiornaliera: 1,
      }));
      ...
      const payload = {
        ...
        farmaciRichiesti: righeRichiesta,
        motivo: data.motivo,
      ...
      toast({
        ...
        description: isFarmaciDaBanco
          ? "La richiesta è stata inviata in modalità farmaci da banco"
          : isAccertamenti
            ? "Il medico riceverà via email la tua richiesta di accertamenti/esami"
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold">Nuova Richiesta al Medico</h1>
        <p className="text-gray-600 mt-2">
          Richiedi farmaci con ricetta, da banco oppure accertamenti/esami specifici
        </p>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Scegli il tipo di richiesta da inviare rapidamente al medico
              </CardDescription>
            </CardHeader>
            <CardContent>
              <FormField
                        ...
                        <SelectItem value="farmaci_da_banco">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-gray-500">
                                Senza ricetta, richiesta rapida OTC
                              </div>
                            </div>
                          </div>
                        </SelectItem>
                        <SelectItem value="accertamenti_esami">
                          <div className="flex items-center gap-2">
                            <Stethoscope className="h-4 w-4" />
                            <div>
                              <div className="font-medium">Accertamenti / Esami</div>
                              <div className="text-xs text-gray-500">
                                Richiesta specialistica inviata al medico via email
                              </div>
                            </div>
                          </div>
                        </SelectItem>
                    ...
                    <FormDescription>
                      {tipoRichiesta === "farmaci"
                        ...
                        : tipoRichiesta === "farmaci_da_banco"
                          ? "Richiesta OTC senza necessità di ricetta medica"
                          : "La richiesta di accertamenti/esami verrà inviata al medico via email"}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              {tipoRichiesta === "farmaci_da_banco" && (
                <Alert className="mt-4 bg-blue-50 border-blue-200">
                  ...
                  <AlertDescription className="text-blue-800">
                    ...
                    Usa questa modalità per richieste rapide non soggette a prescrizione.
                  </AlertDescription>
                </Alert>
              )}

              {tipoRichiesta === "accertamenti_esami" && (
                <Alert className="mt-4 bg-emerald-50 border-emerald-200">
                  <AlertCircle className="h-4 w-4 text-emerald-700" />
                  <AlertDescription className="text-emerald-900">
                    Questa richiesta è pensata per accertamenti/esami specifici: allega documentazione specialistica
                    e descrivi in modo sintetico cosa serve al medico.
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {isAccertamenti ? "Documentazione Specialistica (Opzionale)" : "Foto Talloncino (Opzionale)"}
              </CardTitle>
              <CardDescription>
                {isAccertamenti
                  ? "Carica referti, impegnative o documenti utili (PDF, DOC, immagini)"
            ...
            <CardContent>
              ...
              >
                <input
                  ...
                  className="hidden"
                  accept={isAccertamenti ? "image/*,.pdf,.doc,.docx,.zip" : "image/*"}
                ...
                {uploadedPhotoUrl ? (
                  <div className="space-y-4">
                    {uploadedFileMimeType?.startsWith("image/") ? (
                      ...
                      <div className="relative w-full max-w-md mx-auto rounded-lg border bg-white p-4">
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-2 text-sm">
                            <FileText className="h-4 w-4 text-blue-600" />
                            <span className="font-medium">{uploadedFileName || "Documento allegato"}</span>
                          </div>
                          <Button
                            type="button"
                            variant="destructive"
                    ...
                    <p className="text-sm text-green-600 font-medium">
                      ✓ {isAccertamenti ? "Documento allegato con successo" : "Foto caricata con successo"}
                    </p>
                    <label htmlFor="talloncino-upload">
                      <Button type="button" variant="outline" size="sm" asChild>
                        <span>
                          <Upload className="h-4 w-4 mr-2" />
                          {isAccertamenti ? "Carica un altro documento" : "Carica un'altra foto"}
                        </span>
                      </Button>
                    </label>
                  </div>
                        ...
                        <>
                          ...
                          <div>
                            <p className="text-sm font-medium text-gray-700">
                              {isAccertamenti
                                ? "Carica documentazione clinica per il medico"
                                : "Scatta o carica una foto del talloncino"}
                            </p>
                            <p className="text-xs text-gray-500 mt-1">
                              {isAccertamenti
                                ? "Trascina il file o clicca per selezionare (max 50MB)"
                          ...
                          <Button type="button" variant="outline" size="sm" disabled={uploadingFile}>
                            <Upload className="h-4 w-4 mr-2" />
                            {isAccertamenti ? "Carica Documento" : "Carica Foto"}
                          </Button>
                        </>
                      )}
                    </div>
          ...
          <Card>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    {isAccertamenti ? "Accertamenti / Esami Richiesti" : "Farmaci Richiesti"}
                  </CardTitle>
                  <CardDescription>
                    {isAccertamenti
                      ? "Aggiungi gli accertamenti o esami specifici da sottoporre al medico"
                      : "Aggiungi i farmaci che desideri richiedere"}
                  </CardDescription>
                </div>
                <Button type="button" variant="outline" size="sm" onClick={addFarmaco}>
                  <Plus className="h-4 w-4 mr-2" />
                  {isAccertamenti ? "Aggiungi Accertamento" : "Aggiungi Farmaco"}
                </Button>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              {farmaci.map((_, index) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <Badge variant="outline">{isAccertamenti ? `Richiesta ${index + 1}` : `Farmaco ${index + 1}`}</Badge>
                    {farmaci.length > 1 && (
                      <Button
                        type="button"
                        variant="ghost"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="md:col-span-2">
                          <FormLabel>{isAccertamenti ? "Nome Accertamento / Esame *" : "Nome Farmaco *"}</FormLabel>
                          <FormControl>
                            <Input
                              placeholder={isAccertamenti ? "Es: Emocromo completo, RM lombare" : "Es: Tachipirina"}
                              {...field}
                            />
                          </FormControl>
                          <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{isAccertamenti ? "Dettagli (opzionale)" : "Dosaggio (opzionale)"}</FormLabel>
                        <FormControl>
                          <Input
                            placeholder={
                              isAccertamenti
                                ? "Es: Prioritario, controllo annuale, indicazione specialista"
                                : "Es: 500mg, 20 compresse"
                            }
                            {...field}
                          />
                        </FormControl>
                        <FormDescription>
                          {isAccertamenti
                            ? "Aggiungi eventuali dettagli utili per il medico"
                        ...
                        placeholder={
                          tipoRichiesta === "farmaci"
                            ...
                            : tipoRichiesta === "farmaci_da_banco"
                              ...
                              : "Es: Ho una richiesta dello specialista per ECG + esami ematici di controllo"
                        }
                        rows={4}
                        {...field}
                      />
                    ...
                    <FormDescription>
                      {tipoRichiesta === "farmaci"
                        ...
                        : tipoRichiesta === "farmaci_da_banco"
                          ...
                          : "Descrivi chiaramente quali accertamenti/esami servono e perché"}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
