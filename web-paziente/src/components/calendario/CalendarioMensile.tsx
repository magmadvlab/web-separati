...
interface DisponibilitaGiorno {
  ...
  stato: 'disponibile' | 'full' | 'non_disponibile';
}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-7 gap-1">
          {days.map((date, index) => {
            ...
            const dateStr = date.toISOString().split('T')[0];
            const disp = getDisponibilitaForDate(date);
            const past = isPast(date);
            const isSelected = dateStr === selectedDate;

            const isAvailable = disp && disp.disponibile && disp.stato === 'disponibile' && !past;
            const isFull = disp && disp.disponibile && disp.stato === 'full' && !past;
              ...
              >
                ...
                {disp && disp.disponibile && !past && (
                  <span className="text-xs mt-1">
                    {disp.posti_disponibili}/{disp.posti_totali}
                  </span>
                )}
              </button>
            );
